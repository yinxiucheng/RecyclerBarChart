package com.yxc.barchart.map.location.service.android;
...
public class AndroidLocationComputeUtil {
    /**
    ...
    public static ArrayList<Location> parseLocations(String latLonStr) {
        ...
        for (int i = 0; i < latLonStrs.length; i++) {
            Location location = AndroidLocationComputeUtil.parseLocation(latLonStrs[i]);
            if (location != null) {
    ...
    //获取两点的距离
    public static double getDistance(Location location, Location lastLocation) {
    ...
    public static boolean isExceptPoint(AMapLocation lastSaveLocation, AMapLocation aMapLocation, int type, long intervalTime) {
        ...
        if (recordLocation != null && recordLocation.duration > 2 * intervalTime) {
            ...
            double distance = AndroidLocationComputeUtil.getDistance(lastSaveLocation, aMapLocation);
            ...
            if (speedActual > AndroidLocationComputeUtil.getMaxSpeedByType(type)) {
                return true;
            }
        }
        return false;
    ...
    public static List<Location> getAMapLocationList(List<RecordLocation> recordLocationList) {
        List<Location> aMapLocationList = new ArrayList<>();
        ...
        for (int i = 0; i < recordLocationList.size(); i++) {
            ...
            Location aMapLocation = AndroidLocationComputeUtil.parseLocation(recordLocation.locationStr);
    ...
    public static List<Location> getAMapLocationList2(List<RecordCorrect> recordLocationList) {
        List<Location> aMapLocationList = new ArrayList<>();
        if (recordLocationList == null) {
            return aMapLocationList;
        }
