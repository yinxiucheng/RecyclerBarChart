...
import com.yxc.barchartlib.component.AxisBase;
import com.yxc.barchartlib.util.MPPointD;
import com.yxc.barchartlib.util.Transformer;
import com.yxc.barchartlib.util.Utils;
import com.yxc.barchartlib.util.ViewPortHandler;
    ...
    protected void computeAxisValues(float min, float max) {
        ...
        if (mAxis.isForceLabelsEnabled()) {
            ...
            if (mAxis.mEntries.size() < labelCount) {
                ...
                mAxis.mEntries = new ArrayList<>(labelCount);
            }

            float v = min;

            for (int i = 0; i < labelCount; i++) {
                mAxis.mEntries.set(i, v);
                v += interval;
            }
            ...
            if (mAxis.mEntries.size() < n) {
                // Ensure stops contains at least numStops elements.
                mAxis.mEntries = new ArrayList<>(n);
            }
        ...
        if (mAxis.isCenterAxisLabelsEnabled()) {
            ...
            for (int i = 0; i < n; i++) {
                mAxis.mCenteredEntries[i] = mAxis.mEntries.get(i) + offset;
            }
        }
    }
